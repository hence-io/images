...
docker run --rm -it -v /etc/ssl:/etc/ssl -e CONFIG_PRESET=nodejs -e NGINX_SSL_CERT_FILE=ssl-cert-snakeoil.pem -e NGINX_SSL_CERT_KEY_FILE=ssl-cert-snakeoil.key -e ENABLE_SSL=1 -p 80:80 --name nginx hence/nginx
